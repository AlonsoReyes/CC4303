...
int getNumSeq(unsigned char *buf) {
    int res=0;
    int i;
...
void *bwss_connect(void *ptr) {
	...
	for(bytes=0;; bytes+=cnt) {
		...
		Dwrite(bwc_socket, final_bwcbuff_buffer, cnt - DHDR); // escritura al socket TCP del cliente
